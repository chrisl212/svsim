...
static void _ast_module_nonansi_header_print(ast_node_t *node, int indent, int indent_incr) {
    ...
    if (module_nonansi_header->package_import_declaration_list) {
        printf("\n");
        ast_node_print(module_nonansi_header->package_import_declaration_list, indent + indent_incr, indent_incr);
    }
    if (module_nonansi_header->parameter_port_list) {
        printf("\n");
        ast_node_print(module_nonansi_header->parameter_port_list, indent + indent_incr, indent_incr);
    }
    if (module_nonansi_header->list_of_ports) {
        printf("\n");
        ast_node_print(module_nonansi_header->list_of_ports, indent + indent_incr, indent_incr);
    }
    printf(";");
}
